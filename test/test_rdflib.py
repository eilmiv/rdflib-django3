...
class ContextTest(TestCase):
    ...
    def testTriples(self):
        ...
        self.assertEqual(
            len(list(c1triples((Any, self.likes, self.pizza)))), 2
        )
        self.assertEqual(
            len(list(c1triples((Any, self.hates, self.pizza)))), 1
        )
        self.assertEqual(
            len(list(c1triples((Any, self.likes, self.cheese)))), 3
        )
        self.assertEqual(
            len(list(c1triples((Any, self.hates, self.cheese)))), 0
        )
        ...
        self.assertEqual(
            len(list(c1triples((self.michel, self.likes, Any)))), 2
        )
        self.assertEqual(
            len(list(c1triples((self.tarek, self.likes, Any)))), 2
        )
        ...
        self.assertEqual(
            len(list(c1triples((self.michel, Any, self.cheese)))), 1
        )
        self.assertEqual(
            len(list(c1triples((self.tarek, Any, self.cheese)))), 1
        )
        ...
        self.assertEqual(
            len(list(triples((self.michel, Any, self.cheese)))), 1
        )
        ...
        for c in [graph, self.get_context(self.c1)]:
            # unbound subjects
            self.assertEqual(
                set(c.subjects(self.likes, self.pizza)),
                {self.michel, self.tarek},
            )
            self.assertEqual(
                set(c.subjects(self.hates, self.pizza)), {self.bob}
            )
            self.assertEqual(
                set(c.subjects(self.likes, self.cheese)),
                {self.tarek, self.bob, self.michel},
            )
            self.assertEqual(set(c.subjects(self.hates, self.cheese)), set())

            # unbound objects
            self.assertEqual(
                set(c.objects(self.michel, self.likes)),
                {self.cheese, self.pizza},
            )
            self.assertEqual(
                set(c.objects(self.tarek, self.likes)),
                {self.cheese, self.pizza},
            )
            self.assertEqual(
                set(c.objects(self.bob, self.hates)), {self.michel, self.pizza}
            )
            self.assertEqual(
                set(c.objects(self.bob, self.likes)), {self.cheese}
            )

            # unbound predicates
            self.assertEqual(
                set(c.predicates(self.michel, self.cheese)), {self.likes}
            )
            self.assertEqual(
                set(c.predicates(self.tarek, self.cheese)), {self.likes}
            )
            self.assertEqual(
                set(c.predicates(self.bob, self.pizza)), {self.hates}
            )
            self.assertEqual(
                set(c.predicates(self.bob, self.michel)), {self.hates}
            )

            self.assertEqual(
                set(c.subject_objects(self.hates)),
                {(self.bob, self.pizza), (self.bob, self.michel)},
            )
            self.assertEqual(
                set(c.subject_objects(self.likes)),
                {
                    (self.tarek, self.cheese),
                    (self.michel, self.cheese),
                    (self.michel, self.pizza),
                    (self.bob, self.cheese),
                    (self.tarek, self.pizza),
                },
            )

            self.assertEqual(
                set(c.predicate_objects(self.michel)),
                {(self.likes, self.cheese), (self.likes, self.pizza)},
            )
            self.assertEqual(
                set(c.predicate_objects(self.bob)),
                {
                    (self.likes, self.cheese),
                    (self.hates, self.pizza),
                    (self.hates, self.michel),
                },
            )
            self.assertEqual(
                set(c.predicate_objects(self.tarek)),
                {(self.likes, self.cheese), (self.likes, self.pizza)},
            )

            self.assertEqual(
                set(c.subject_predicates(self.pizza)),
                {
                    (self.bob, self.hates),
                    (self.tarek, self.likes),
                    (self.michel, self.likes),
                },
            )
            self.assertEqual(
                set(c.subject_predicates(self.cheese)),
                {
                    (self.bob, self.likes),
                    (self.tarek, self.likes),
                    (self.michel, self.likes),
            ...
            self.assertEqual(
                set(c.subject_predicates(self.michel)),
                {(self.bob, self.hates)},
            )
            ...
            self.assertEqual(
                set(c),
                {
                    (self.bob, self.hates, self.michel),
                    (self.bob, self.likes, self.cheese),
                    (self.tarek, self.likes, self.pizza),
                    (self.michel, self.likes, self.pizza),
                    (self.michel, self.likes, self.cheese),
                    (self.bob, self.hates, self.pizza),
